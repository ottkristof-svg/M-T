...
// Merülés-költség kalkulátor (saját interaktív program)
...
if (costForm && calcResult) {
  costForm.addEventListener("submit", (event) => {
    ...
    if (days <= 0 || divesPerDay <= 0 || pricePerDive < 0 || equipmentPrice < 0 || otherCosts < 0) {
    ...
    calcResult.textContent =
      `Összesen ${totalDives} merülésre kb. ${total.toFixed(0)} EUR költséggel számolhatsz (fiktív példa).`;
  });
}

// Kapcsolat űrlap – JS validálás (legalább 5 mező)

const contactForm = document.getElementById("contactForm");
if (contactForm) {
  const showError = (name, message) => {
    const field = contactForm.elements[name];
    const errorEl = contactForm.querySelector(`.error-message[data-for="${name}"]`);
    if (field) {
  ...
  const validateEmail = (value) => {
    // egyszerűbb minta, oktatási célra
  ...
  contactForm.addEventListener("submit", (event) => {
    ...
    const interestValue = contactForm.querySelector("input[name='interest']:checked");
    ...
    if (!acceptPolicyChecked) {
      showError("acceptPolicy", "A folytatáshoz el kell fogadnod az adatkezelési tájékoztatót.");
      isValid = false;
    }
    ...
    // Ha minden rendben, "elküldöttnek" tekintjük
    if (formSuccess) {
      formSuccess.textContent = "Köszönjük az üzeneted! Hamarosan felvesszük veled a kapcsolatot (fiktív példa).";
    }
    contactForm.reset();
  });
}
